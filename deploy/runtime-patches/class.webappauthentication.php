<?php
/*
 * MassPanel modification, 2026-08-28.
 * Adds administrator-assisted mailbox access and suspended-mailbox handling.
 * Based on grommunio Web and distributed under AGPL-3.0-or-later.
 * Corresponding source: /masspanel-source on the configured mail hostname.
 */
...
class WebAppAuthentication {
	...
	public static function getErrorMessage() {
		if (!empty($_SESSION['masspanel_suspended_login'])) {
			return _('This mailbox is suspended. Incoming mail is still being received, but webmail and outgoing mail are disabled. Contact your hosting provider.');
		}
	...
	*/
	...
	public static function authenticateWithPostedCredentials() {
		...
		$_SESSION['masspanel_suspended_login'] = false;
		$suspendedFile = '/etc/grommunio-web/masspanel-suspended-mailboxes';
		$suspendedMailboxes = is_readable($suspendedFile) ? file($suspendedFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
		if (in_array(strtolower($email), $suspendedMailboxes, true)) {
			$_SESSION['masspanel_suspended_login'] = true;
